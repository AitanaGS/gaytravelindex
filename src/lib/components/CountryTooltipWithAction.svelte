<script>
    ...
    $: xPosition = yearScale(data.year) + margin.left + yearScale.bandwidth() / 2 + tooltipWidth /2 > innerWidth + margin.left
        ...
        : yearScale(data.year) + margin.left + yearScale.bandwidth() / 2 - tooltipWidth / 2 < margin.left + tooltipWidth / 2
        ? yearScale(data.year) + margin.left
        : yearScale(data.year) + margin.left + yearScale.bandwidth() / 2 - tooltipWidth / 2

    $: yPosition = countryScale(data.country) - tooltipHeight / 2 < margin.top
        ? countryScale(data.country) + tooltipHeight - 5
        : countryScale(data.country) + margin.top - tooltipHeight

    $: maxYPosition = countryScale(data.country)

    $: flyDirection = yPosition < maxYPosition ? 1 : -1

    $: console.log(data.country, yPosition, flyDirection)
...
<h3>{data.country} {data.year}</h3>
    <div class="info">
        <span 
            class="total"
            style="
                background: {totalScale(data.total)};
